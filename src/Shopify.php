...
namespace arslanramay\ShopifyPHP;
...
class Shopify
{
    ...
    /**
     * @var string The API Key used for requests
     */
    private $api_key;
    ...
    /**
     * @var GuzzleHttp\Client The GuzzleHttp client used for making HTTP requests
     */
    private $client;

    /**
     * @var array Stores the headers from the last request for tracking rate limits
     */
     ...
     * Sets the token and reinitializes the client to ensure the new token is used for future requests
     *
     ...
     *
     * Generates the URL for the OAuth authorization request, including client ID, scopes, redirect URI, and state (nonce). Optionally requests an online access mode
     *
     ...
     * @return string $url The Authorization URL for OAuth request
     */
    ...
    /**
     * Main function to make API calls to Shopify
     *
     * @param string $method The HTTP method to use for the request(GET, POST, PUT, DELETE)
     * @param string $endpoint The endpoint to access on Shopify
     * @param array $params The parameters to provide in the API request
     * @return mixed $response Returns the response body from Shopify. Typically an object or array
     */
    public function call(string $method, string $endpoint, $params = [])
    {
        ...
        // Prepare the request based on the HTTP method used
        switch ($method) {
            case 'GET':
            case 'DELETE':
        ...
        // Initiate the request to Shopify
        $response = $this->client->request($method, $endpoint, $options);

        // Store the response headers from Shopify for later usage
